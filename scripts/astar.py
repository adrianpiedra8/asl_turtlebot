...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        directions = np.array(directions)*self.resolution
        for i in range(directions.shape[0]):
            x2 = (x[0] + directions[i,0], x[1] + directions[i,1])
            x2 = self.snap_to_grid(x2)
            if self.is_free(x2):
                neighbors.append(x2)
    ...
    def plot_path(self, fig=0, name1='', name2='', pcolor='green'):
        ...
        plt.annotate(name1, np.array(self.x_init)*self.resolution + np.array([.2, 0]), fontsize=16)
        plt.annotate(name2, np.array(self.x_goal)*self.resolution + np.array([.2, 0]), fontsize=16)
    ...
    def solve(self):
        while len(self.open_set)>0:
            ...
            for x_neigh in self.get_neighbors(x_current):
                ...
                # If neighbor already in open set check g score for neighbor and skip if
                # existing g score was lower
